...
export default function Simulacros({ data, role }: { data: AppData; role: string }) {
  const D = data;
  const isTeacher = role === 'teacher';
  const last = D.SIMULATIONS.find(s => s.score !== null) ?? D.SIMULATIONS[0];
  const r = 52;
  const circ = 2 * Math.PI * r;
  const lastScore = last?.score ?? 0;
  const lastAciertos = last?.score && last?.questions
    ? Math.round(last.score * last.questions / 100)
    : null;
    ...
    <div className="content content--narrow fade-in">
      ...
      {D.SIMULATIONS.length === 0 ? (
        <div className="card" style={{ textAlign: 'center', padding: '48px 20px', color: 'var(--text-muted)' }}>
          ...
          <div style={{ fontWeight: 500 }}>No hay simulacros aún</div>
          <div style={{ fontSize: 13, marginTop: 4 }}>Los simulacros aparecerán aquí cuando estén disponibles.</div>
        </div>
      ) : (
        <>
          <div className="two-col" style={{ marginBottom: 24 }}>
            <div className="card">
              <div className="section-title">
                ...
                {last && <span className="tag">{last.name}</span>}
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 24, marginBottom: 18 }}>
                <div style={{ position: 'relative', width: 120, height: 120, flexShrink: 0 }}>
                  <svg viewBox="0 0 120 120" width="120" height="120">
                    ...
                    <circle cx="60" cy="60" r={r} fill="none" stroke="var(--blue-500)" strokeWidth="10"
                      ...
                      strokeDashoffset={`${circ * (1 - lastScore / 100)}`}
                      transform="rotate(-90 60 60)" strokeLinecap="round" />
                  </svg>
                  <div style={{ position: 'absolute', inset: 0, display: 'grid', placeItems: 'center', textAlign: 'center' }}>
                    <div>
                      <div style={{ fontFamily: 'var(--font-serif)', fontSize: 34, lineHeight: 1 }}>
                        {last?.score ?? '—'}
                      </div>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 14 }}>
                    {last?.date} · {last?.dur} · {last?.questions} reactivos
                  </div>
                  {lastAciertos !== null && (
                    <div style={{ display: 'flex', gap: 18, fontSize: 12 }}>
                      <div>
                        <div style={{ color: 'var(--text-muted)', fontSize: 11 }}>Aciertos</div>
                        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 16, marginTop: 2 }}>
                          {lastAciertos}/{last?.questions}
                        </div>
                      </div>
                      <div>
                        <div style={{ color: 'var(--text-muted)', fontSize: 11 }}>Calificación</div>
                        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 16, marginTop: 2, color: 'var(--acc-emerald)' }}>
                          {last?.score}/100
                        </div>
                      </div>
                    </div>
            ...
            <div className="card">
              ...
              {D.SIM_BREAKDOWN.length === 0 ? (
                <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>Sin desglose disponible.</div>
              ) : D.SIM_BREAKDOWN.map(s => (
                <div key={s.area} className="sim-bar">
                  <div className="sim-bar__lbl">{s.area}</div>
                  <div className="hbar">
                    <div className="hbar__fill" style={{
                      width: s.pct + '%',
                      background: s.pct >= 85 ? 'var(--acc-emerald)' : s.pct >= 75 ? 'var(--blue-500)' : 'var(--acc-amber)'
                    }}></div>
                  </div>
                ...
                {D.SIMULATIONS.map(s => (
                  <tr key={s.id}>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      {s.score !== null
                        ? <span className="score">{s.score}<span className="score__pct">/100</span></span>
